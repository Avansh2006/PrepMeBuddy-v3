  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6 flex flex-col items-center">
        ...
        {!active ? (
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p
                className={`text-lg font-bold ${
                  timer <= 30 ? "text-red-400" : "text-teal-400"
                }`}
              >
            ...
            <textarea
              ...
              className="w-full p-3 border border-gray-600 rounded-lg bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent resize-none h-32"
            />

            {/* Feedback */}
            {showFeedback && (
              <p className="text-sm text-gray-400 italic">{getFeedback()}</p>
            )}
            ...
            <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0">
              ...
              <button
                ...
                className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:bg-gray-500 transition-colors"
              >
                Check Feedback
              </button>
              <button
      ...
      {(!active && answers.length > 0) && (
        <div className="w-full max-w-2xl mt-6 bg-gray-800 rounded-xl shadow-lg p-6">
          ...
          <ul className="space-y-3">
            {answers.map((item, idx) => (
              <li key={idx} className="p-3 bg-gray-700 rounded-lg">
                <strong className="text-teal-400">Q:</strong> {item.question}
                <br />
                <strong className="text-teal-400">A:</strong> {item.answer}
              </li>
            ))}
          </ul>
          <button
